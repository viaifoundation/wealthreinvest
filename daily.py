...
def generate_klines(ticker='NVDA', step=5, start_time_str='0930'):
    ...
    # Filter from start time to now (assume UTC in index)
    ...
    if isinstance(current, (int, float, np.floating)) and isinstance(open_price, (int, float, np.floating)) and open_price != 0:
        ...
        pct_change = 0
    sign = '+' if pct_change > 0 else '-'
    if isinstance(current, (int, float, np.floating)) and current == current:
        current_str = f"{current:10.2f}"
    else:
        current_str = f"{'N/A':>10}"
